...
HEADER = ['uid', 'pid', 'domain', 'label', "reach_time"]
...
def save_train_val_test_by_time(domain_save_path, df, conf):
    train_data_path, val_data_path, test_data_path = [osp.join(domain_save_path, path) for path in
                                                      ['train.csv', 'val.csv', 'test.csv']]
    ...
    with open(train_data_path, "a", encoding='utf8', newline='') as train, open(val_data_path, "a", encoding='utf8',
        ...
        train_writer, val_writer, test_writer = csv.writer(train), csv.writer(val), csv.writer(test)
        df = df.sort_values("reach_time")
        size = len(df)
        eval_start = int(size * conf['train_val_test'][0])
        df_train = df[:eval_start]
        df_eval_test = df[eval_start:]
        df_val, df_test, _, _ = train_test_split(df_eval_test,
                                              df_eval_test[['label']],
                                              stratify=df_eval_test[['label']],
                                              test_size=conf['train_val_test'][2]/(conf['train_val_test'][1]+conf['train_val_test'][2]),
...
def split_by_theme(processed_file_list, split_save_path, conf):
    n_domain = 0
    for p in processed_file_list:
        ...
        if not file_exist(domain_save_path) or conf['rebuild']:
            ...
            df = pd.read_csv(p)
            df = df.rename(columns={"user_id": "uid", "item_id": "pid"})
            ...
            save_train_val_test_by_time(domain_save_path, df, conf)
...
def split_to_domains(conf):
    ...
    processed_file_list = preprocess_data1.preprocess(processed_data_path, raw_data_path, theme_num=conf['theme_num'],
                                                     rebuild=conf['rebuild'])
    n_domain = 0
    if conf['split_policy'] == "split_by_theme":
        n_domain = split_by_theme(processed_file_list, split_save_path, conf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, help="split config file", default="config_10.json", required=False)
