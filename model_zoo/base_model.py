...
class BaseModel(object):
    ...
    def val_and_test_total(self, mode):
        ...
        '''
        if mode == "val":
            dataset = self.dataset.total_val_dataset
        elif mode == "test":
            dataset = self.dataset.total_test_dataset
            self.load_model(self.checkpoint_path)  # Load best model weights
        else:
            raise ValueError("Mode can be either val or test, not: {}".format(mode))

        loss, auc = self.model.evaluate(dataset['data'], steps=dataset['n_step'],
                                            verbose=0)
        print("Overall {} Loss: {}, AUC: {}".format(mode, loss, auc))
        return loss, auc
    ...
    def early_stop_step(self, metric):
        ...
        if self.best_metric is None:
            ...
            #
            # if self.counter >= self.patience and self.epoch >= 25:
            #     self.early_stop = True
            # if self.counter >= self.patience and self.epoch < 25:
            #     self.counter = 0
        else:
