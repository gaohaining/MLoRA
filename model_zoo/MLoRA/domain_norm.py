...
class DomainNorm(Layer):
    def __init__(self,
                 ...
                 **kwargs):
        super(DomainNorm, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        # Partitioned Part
        self.domain_gamma = self.add_weight(shape=shape,
                                            name="gamma_specific",
                                            initializer=self.gamma_initializer,
                                            regularizer=self.gamma_regularizer,
                                            constraint=self.gamma_constraint)
        self.domain_beta = self.add_weight(shape=shape,
                                           name="beta_specific",
        ...
        for idx in range(self.n_domain):
            self.domain_mean.append(self.add_weight(
                shape=(dim,),
            ...
            self.domain_var.append(self.add_weight(
                shape=(dim,),
    ...
    def _domain_partition(self, idx):
        d_gamma = nn.embedding_lookup(self.domain_gamma, idx)
        d_beta = nn.embedding_lookup(self.domain_beta, idx)

        # self.gamma = tf.math.sqrt(tf.math.pow(self.shared_gamma, 2) + tf.math.pow(d_gamma, 2))
        self.gamma = tf.math.sqrt(tf.abs(self.shared_gamma) + tf.abs(d_gamma))
        self.beta = self.shared_Beta + d_beta
        ...
        for i in range(self.n_domain):
            mean_paris.append((tf.equal(idx, i), (lambda k: lambda: self.domain_mean[k])(i)))
            var_paris.append((tf.equal(idx, i), (lambda k: lambda: self.domain_var[k])(i)))
    ...
    def call(self, inputs, training=None):
        ...
        mean_update_paris = []
        var_update_paris = []
        print("self.domain_mean[k]: ", self.domain_mean[0])
        print("mean: ", mean)
        for i in range(self.n_domain):
            mean_update_paris.append(
                (tf.equal(idx, i), (lambda k: lambda: K.moving_average_update(self.domain_mean[k], mean,
                                                                              self.momentum))(i)))
            var_update_paris.append((tf.equal(idx, i), (
                lambda k: lambda: K.moving_average_update(self.domain_var[k], variance,
                                                          self.momentum))(i)))
    ...
    def get_config(self):
        ...
        base_config = super(DomainNorm, self).get_config()
