...
from model_zoo.DeepCTR.mlora_nodomain import Mlora
...
class DeepCTR(BaseModel):
    ...
    def build_model(self):
        ...
        if 'lora' in self.model_config['name']:
            model = self.build_mlp_lora(dnn_feature_columns,
                                   dnn_hidden_units=self.model_config['hidden_dim'],
                                   dnn_dropout=self.model_config['dropout'])
            ...
            model = models.DCN(linear_feature_columns, dnn_feature_columns,
                                  dnn_hidden_units=self.model_config['hidden_dim'],
                                  dnn_dropout=self.model_config['dropout'])
        elif "ifm" in self.model_config['name']:
            model = models.IFM(linear_feature_columns, dnn_feature_columns,
                                  dnn_hidden_units=self.model_config['hidden_dim'],
                                  dnn_dropout=self.model_config['dropout'])
        elif "xdeepfm" in self.model_config['name']:
            model = models.xDeepFM(linear_feature_columns, dnn_feature_columns,
                                  dnn_hidden_units=self.model_config['hidden_dim'],
                                  dnn_dropout=self.model_config['dropout'])
        elif "fibinet" in self.model_config['name']:
            model = models.FiBiNET(linear_feature_columns, dnn_feature_columns,
                                  dnn_hidden_units=self.model_config['hidden_dim'],
                                  dnn_dropout=self.model_config['dropout'])
        elif "difm" in self.model_config['name']:
            model = models.DIFM(linear_feature_columns, dnn_feature_columns,
                                  dnn_hidden_units=self.model_config['hidden_dim'],
                                  dnn_dropout=self.model_config['dropout'])
        model.summary()
        # Optimization
    ...
    def train(self):
        ...
        for epoch in range(self.train_config['epoch']):
            ...
            for idx in train_sequence:
                ...
                old_time = time.time()
                self.model.fit(d['data'], steps_per_epoch=d['n_step'], verbose=1, callbacks=[],
                               epochs=epoch + 1, initial_epoch=epoch)
                # pre = self.model.predict(d['data'], steps=d['n_step'], verbose=1)
                # print(pre)
    ...
    def build_mlp_lora(self, dnn_feature_columns, dnn_hidden_units=(256, 128, 64), l2_reg_linear=0.00001,
                  l2_reg_embedding=0.00001, l2_reg_dnn=0, seed=1024, dnn_dropout=0., dnn_activation='relu',
                  task='binary'):
        features = build_input_features(dnn_feature_columns)
        ...
        dnn_out = Mlora(self.n_domain, dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, False, seed=seed, lora_reduce = self.model_config['lora_reduce'])(
